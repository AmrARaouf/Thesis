...
\chapter{Events}
\label{chap:Events}
Events are the backbone of the software workings of this thesis. Raw sensor data are converted to events that can be further processed in the software's pipeline. Furthermore external software send event's to ours over a network. These events can be used by themselves as output or can be considered as further inputs to trigger other events.

\section{Sensors}
\label{sec:Sensors}
A number of sensors are used to measure the subject's verbal and nonverbal data. They are described here in details
